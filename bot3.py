...
# Multi-checkpoint growth tracking (replaces the old single fixed check):
...
def format_confirmed_alert(data, holder_count, elapsed_seconds, label):
...
async def check_confirmation_later(data):
    ...
    for checkpoint_time, threshold, label in CHECKPOINTS:
        ...
        if holder_count >= threshold:
            ...
            print(f"{label}: {symbol} - {holder_count} holders at {checkpoint_time}s")
            message = format_confirmed_alert(data, holder_count, checkpoint_time, label)
